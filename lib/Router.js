  ...
  function next(err, nskip) {
    ...
    if (err !== undefined && err !== null) {
      if (isError(err))
        ...
        if (err === 'skip' && typeof nskip === 'number')
          ptr += nskip;
        else {
          error = new Error(err);
    ...
    for (; ptr < stacklen; ++ptr, paramPtr = 0) {
      ...
      var handlerLen = handler.length;
      if (!((error === undefined && handlerLen < 4)
            || (error !== undefined && handlerLen === 4)))
        continue;
      ...
      // increment route handler index in case current handler immediately
      // calls next()
      ++ptr;
      ...
      if (ret && isError(ret)) {
        error = ret;
        // reset route handler index otherwise the next handler will be skipped
        // because of our previous, early increment
        --ptr;
        continue;
      }
      return;
...
function execHandler(handler, req, res, next, error) {
  try {
    ...
    console.error('!!! route handler exception:');
...
function execParam(fn, req, res, next, val) {
  try {
    ...
    console.error('!!! route param exception:');
    console.error(ex.stack);
    ex.code = 500;
    return ex;
  }
...
Router.prototype.use = function use(route) {
  ...
  for (var i = 0, len = routes.length, fullPath, regex, keys; i < len; ++i) {
    ...
    if (isRegExp(routes[i])) {
      fullPath = this.path;
      if (this.config['concat regex'] !== false)
        routes[i] = concatRegex(this.path, routes[i]);
      regex = pathRegex(routes[i], keys);
    } else if (routes === ROUTES_MATCH_ALL) {
      fullPath = this.path;
    ...
    for (var h = handlersStart; h < handlersEnd; ++h) {
      ...
      if (typeof handlerfn !== 'function' && !(handlerfn instanceof Router))
        ...
        if (handlerfn._mounted)
          throw new Error('Cannot mount a Router in multiple places');
        if (routes !== ROUTES_MATCH_ALL && !isRegExp(routes[i]))
          handlerRegex = pathRegex(fullPath, keys, { end: false });
        handlerfn.path = fullPath;
        handlerfn.config = this.config;
        handlerfn.cache = this.cache;
        handlerfn.engines = this.engines;
        handlerfn.locals = this.locals;
...
METHODS.forEach(function(method) {
  Router.prototype[method] = function methodRoute() {
...
function concatRegex(path, re) {
  var base = path,
      resrc = re.source.replace(/\\/g, ''),
      flags = '';

  // strip off ending '/' because most people will most likely start regexps
  // with a '/'
  if (base.substr(-1) === '/')
    base = base.slice(0, -1);
  ...
  return new RegExp('^' + base + resrc, flags);
}