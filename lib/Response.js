...
var contentDisposition = require('content-disposition'),
    ...
    send = require('send'),
    mimeLookup = send.mime.lookup,
    charsetsLookup = send.mime.charsets.lookup,
    vary = require('vary');
...
Response.prototype.links = function links(links) {
  var link = this.res.getHeader('Link') || '';
  if (link)
    link += ', ';
  var keys = Object.keys(links);
  for (var i = 0, len = keys.length, rel; i < len; ++i) {
    rel = keys[i];
    link += '<';
    link += links[rel];
    link += '>; rel="';
  ...
  return this.setHeader('Link', link);
};
...
Response.prototype.json = function json(obj) {
  ...
  if (arguments.length === 2) {
    ...
    if (typeof arguments[1] === 'number')
      this.res.statusCode = arguments[1];
    else {
      this.res.statusCode = arguments[0];
      val = arguments[1];
    }
  }
...
Response.prototype.jsonp = function jsonp(obj) {
  var req = this.req,
      res = this.res,
      val = obj;
...
Response.prototype.type = function(type) {
  var val = (~type.indexOf('/') ? type : mimeLookup(type));
...
Response.prototype.header = function header(field, val) {
  if (arguments.length === 2) {
    ...
    if (field.toLowerCase() === 'content-type' && !RE_CHARSET2.test(val)) {
      ...
      if (~idx)
        charset = charsetsLookup(val.substring(0, idx));
      else
        charset = charsetsLookup(val);
    ...
    for (var k = 0, len = keys.length, key; k < len; ++k) {
      key = keys[k];
...
Response.prototype.cookie = function(name, val, options) {
  ...
  var headerVal = cookieSerialize(name, '' + value, options);
...
Response.prototype.vary = function vary(field) {
...
Response.prototype.render = function render(view, options, fn) {
  var config = this.router.config,
      opts = options || {},
      self = this;
  ...
  //this.router.render(view, opts, fn);
  render(view, opts, fn);
};

function render(view, options, fn) {
